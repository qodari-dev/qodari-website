...
import { FounderSection } from "@/sanity/types";
  ...
  return (
    <section className={cn("relative overflow-hidden", bg, text)}>
      ...
      <div className="site-container relative">
        ...
        <div
          className={cn(
            "mx-auto rounded-[2rem] border border-black/6 bg-white/94 shadow-[0_18px_45px_rgba(16,24,40,0.06)]",
            hasPhoto ? "max-w-5xl" : "max-w-3xl",
          )}
        >
          <div
            className={cn(
              "flex flex-col",
              hasPhoto && "gap-0 lg:flex-row",
            )}
          >
            {/* Photo */}
            {hasPhoto ? (
              <div className="relative shrink-0 overflow-hidden rounded-t-[2rem] lg:w-[260px] lg:rounded-l-[2rem] lg:rounded-tr-none">
                <Image
                  src={urlFor(photo).width(520).height(680).fit("crop").url()}
                  alt={name}
                  width={520}
                  height={680}
